...
public class Linker {
    ...
    private boolean linkDomains(Domain d1, Domain d2, int level, boolean logging, boolean occurrs) {
        if (level >= d1.getPredicate().getRange()) {
            ...
            int i = level;
            if (d1.get(i).isFSet() && !d2.get(i).isEmpty()) {
                TSubst s = d1.get(i).getF().setResult(d2.get(i).getValue());
                s.setSolves(d1, d2);
                d1.setUsed(true);
                occurrs = true;
            }
            if (d2.get(i).isFSet() && !d1.get(i).isEmpty()) {
                TSubst s = d2.get(i).getF().setResult(d2.get(i).getValue());
                s.setSolves(d2, d1);
                d1.setUsed(true);
                occurrs = true;
            }
