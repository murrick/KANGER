...
//    }
    ...
    private static void showPredRecurse(Mind mind, int index, List<TVariable> list, Domain d, boolean showCauses) {
        if (index >= list.size()) {
            System.out.printf("\t%s\n", d.toString());
            if (showCauses) {
                showCauses(mind, d, 0);
    ...
    public static void showPred(Mind mind, Predicate p, boolean showCauses) {
        ...
        if (set.isEmpty()) {
            ...
            for (Domain s : set) {
                if (!s.isDest()) {
                    showPredRecurse(mind, 0, new ArrayList<>(s.getTVariables()), s, showCauses);
                }
            }
        }
    }
